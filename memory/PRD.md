...
- **JSON handling (Sarvam compatibility)**: `response_format: json_object` REMOVED (Sarvam doesn't support it). System prompt now explicitly requires raw JSON only (no markdown/backticks). Response parsing strips ```json/``` fences and slices from first `{` to last `}` before `JSON.parse` — unit-tested against fenced, conversational-wrapped, and raw outputs.
